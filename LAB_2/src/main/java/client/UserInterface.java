...
public class UserInterface {
    ...
    public void displayWelcome() {
        System.out.println("\n" + "=".repeat(60));
        System.out.println("      Hii!!  Welcome to " + config.getShopName() + "! ");
    ...
    public void displayGoodbye() {
        System.out.println("\n" + "=".repeat(60));
        System.out.println("Thank you for visiting " + config.getShopName() + "come again" + "!");
        System.out.println("=".repeat(60));
    }

    public void displayCoffeeCharacteristics(Coffee coffee) {
    ...
    public void displayFinalReceipt(List<Coffee> order, boolean hasLoyaltyDiscount) {
        System.out.println("\n" + "=".repeat(60));
    ...
    private boolean verifyMembership() {
        ...
        while (attempts < MAX_ATTEMPTS) {
            ...
            if (config.isMember(phoneNumber)) {
                System.out.println(" Membership is verified! Welcome back!");
    ...
    private boolean offerMembership() {
        config.displayLoyaltyInfo();
    ...
    public Coffee createCustomCoffee() {
        ...
        displayCoffeeCharacteristics(factory.createCoffee());
        ...
        if (response.equals("yes") || response.equals("y")) {
            ...
            // Use CUSTOMIZABLE BUILDER
            Coffee baseCoffee = factory.createCoffee();
            return buildCoffee(baseCoffee);
        }
    }

    private Coffee buildCoffee(Coffee baseCoffee) {
        CoffeeBuilder builder = new CoffeeBuilder(baseCoffee);

        builder.withSize(selectSize());
        ...
        builder.withShots(selectShots(baseCoffee));
        addExtras(builder);
    ...
    private CoffeeFactory selectCoffeeType() {
    ...
    private int selectShots(Coffee coffee) {
    ...
    private void addExtras(CoffeeBuilder builder) {
        System.out.println("\n Available Extras:");
